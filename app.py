...
if "params" not in st.session_state:
    st.session_state.params = {
        ...
        "red_flag_3": False,
        "red_flag_4": True,
        "time_in_relationship": 3,  # 单位：月，初始3个月
    }
...
time_in_relationship = st.slider("交往时长（月）", 0, 72, st.session_state.params["time_in_relationship"], key="time_in_relationship")

# 显示“多少个月”或“多少年以上”
if time_in_relationship >= 12:
    years = time_in_relationship // 12
    months = time_in_relationship % 12
    if months == 0:
        time_display = f"{years} 年"
    else:
        time_display = f"{years} 年 {months} 个月"
else:
    time_display = f"{time_in_relationship} 个月"

st.write(f"交往时长：{time_display}")
...
plt.figtext(0.5, 0.05, f"交往时长：{time_display}", ha='center', fontsize=12, fontproperties=my_font)
